...
```
SELECT *, amount - LAG(amount) OVER(PARTITION BY fk_employee ORDER BY from_date) as diff
FROM salary;
...
-------------+--------+------------+------------+----------+--------
           ...
           1 | 600000 | 2024-03-01 | 2099-12-31 |        3 | 400000
...
```
SELECT *, COALESCE(amount - LAG(amount) OVER(PARTITION BY fk_employee ORDER BY from_date), 0) as diff
FROM salary;
 fk_employee | amount | from_date  |  to_date   | fk_grade |  diff  
-------------+--------+------------+------------+----------+--------
